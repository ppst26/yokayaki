...
### Phase 5a — 5 roles + SQL matrix `🟢`

- [x] Task 1: Migration role enum (`staff`→`cashier`) — `20260911_m5_roles_enum.sql`
- [x] Task 2: Permission helpers — `20260912_m5_permission_helpers.sql` (`can_operate_pos` … `can_manage_employees`)
- [x] Task 3: RLS rewrite — `20260913_m5_rls_role_matrix.sql` (policy ใหม่ทุกตาราง)
- [x] Task 4: RPC role guards — `20260914_m5_rpc_role_guards.sql`
- [x] Task 5: `lib/permissions.ts` + JWT/types — `canAccessTab` · `EmployeeRole` 5 ค่า
- [x] Task 6: UI tab gating — `SidebarNav` · `TableMap` · `EmployeeManager`
- [x] Task 7: `requireManageEmployees()` — manager + owner จัดการพนักงาน
- [x] Task 8: Integration tests — `supabase/tests/role_matrix.sql` + อัปเดต `rls_policies.sql` · `employees_rpc.sql` · `a7_audit.sql` · `security.sql`
- [x] Task 9: Types + milestone partial — `lib/database.types.ts` (18 tables · 29 functions รวม `can_*`) · verification ครบ

...
-- =============================================================
-- M6 / G7 — Double points day (config ต่อ org)
-- =============================================================

ALTER TABLE public.org_settings
  ADD COLUMN IF NOT EXISTS double_points_enabled BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS double_points_dates JSONB NOT NULL DEFAULT '[]'::JSONB;

CREATE OR REPLACE FUNCTION public.is_double_points_active(p_org_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  SELECT os.double_points_enabled, os.double_points_dates, os.timezone
  INTO v_enabled, v_dates, v_tz
  ...
  v_today := (NOW() AT TIME ZONE COALESCE(NULLIF(TRIM(v_tz), ''), 'Asia/Bangkok'))::DATE;

  RETURN EXISTS (
    SELECT 1
    FROM jsonb_array_elements_text(COALESCE(v_dates, '[]'::JSONB)) AS d(val)
    WHERE d.val::DATE = v_today
  );
END;
$dp$;

REVOKE EXECUTE ON FUNCTION public.is_double_points_active(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.is_double_points_active(UUID) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.update_double_points_settings(
  p_enabled BOOLEAN,
  p_dates   JSONB
) RETURNS VOID
LANGUAGE plpgsql
...
DECLARE
  ...
  v_date TEXT;
BEGIN
  IF v_org IS NULL OR NOT public.can_manage_loyalty() THEN
  ...
  FOR v_date IN SELECT jsonb_array_elements_text(p_dates)
  LOOP
    BEGIN
      PERFORM v_date::DATE;
    EXCEPTION WHEN OTHERS THEN
      RAISE EXCEPTION 'invalid_date_format: %', v_date;
    END;
  END LOOP;

  UPDATE org_settings
  SET double_points_enabled = COALESCE(p_enabled, FALSE),
      double_points_dates   = p_dates
  WHERE org_id = v_org;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'org_settings_not_found';
  END IF;
END;
$upd$;

REVOKE EXECUTE ON FUNCTION public.update_double_points_settings(BOOLEAN, JSONB) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_double_points_settings(BOOLEAN, JSONB) TO authenticated, service_role;
...
BEGIN
  ...
  IF v_member_pts IS NOT NULL THEN
    ...
    IF public.is_double_points_active(v_org_id) THEN
      r_points_earn := r_points_earn * 2;
    END IF;
  END IF;

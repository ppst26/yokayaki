...
-- =============================================================
-- M5 / 5a — Operational RPC role guards
-- =============================================================
...
COMMENT ON FUNCTION public.place_order_batch(UUID, JSONB) IS
  'สั่งหลายรายการจาก POS — ตรวจ table org_id ตรงกับ JWT + stamp org_id + role guard';
...
COMMENT ON FUNCTION public.place_order_item(UUID, INT, INT, VARCHAR) IS
  'สั่งอาหารจากเครื่อง POS — ตรวจ table org_id ตรงกับ JWT + stamp org_id + role guard';
...
COMMENT ON FUNCTION public.complete_checkout(INT, DECIMAL, VARCHAR, VARCHAR, INT) IS
  'ปิดบิล — ยอดเงินทุกตัวคำนวณจาก order_items + promotions ใน DB + ตรวจ org_id ตรงกับ JWT + role guard';
...
BEGIN
  ...
  IF NOT public.can_kitchen() THEN
    RETURN FALSE;
  END IF;
...
COMMENT ON FUNCTION public.void_order_item(INT, TEXT, TEXT, INT) IS
  'ยกเลิกรายการ — คืนสต็อกตามรหัสเหตุผล บันทึก audit และตรวจ org_id ตรงกับ JWT + role guard';
...
COMMENT ON FUNCTION public.adjust_loyalty_points(VARCHAR, INT, TEXT) IS
  'ปรับแต้มสมาชิกใน transaction เดียว — ล็อกแถว + audit จาก JWT + ตรวจ org_id + role guard';
...
COMMENT ON FUNCTION public.upsert_purchase_order(BIGINT, DATE, VARCHAR, TEXT, JSONB) IS
  'สร้าง/แก้ PO + รายการวัตถุดิบใน transaction เดียว (L6) + stamp org_id จาก JWT + role guard';

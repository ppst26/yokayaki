    ...
    <div className="space-y-6 font-sans w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200/80 p-5 rounded-2xl shadow-sm">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl text-xs font-extrabold text-slate-700 hover:text-slate-900 transition active:scale-95 cursor-pointer flex items-center gap-2 shadow-xs"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div
          ...
          className={`bg-white border rounded-2xl p-5 shadow-sm flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] ${
            activeSubTab === 'sales' ? 'border-blue-500 ring-2 ring-blue-500/10' : 'border-slate-200/80 hover:border-blue-300'
          }`}
        >
          <div className="space-y-1">
            <div className="text-slate-500 text-xs font-semibold">จำนวนบิลปิดแล้ว</div>
            <div className="text-2xl font-black text-slate-900">{totalBills} <span className="text-xs font-bold text-slate-400">บิล</span></div>
          </div>
        ...
        <div
          onClick={() => setActiveSubTab('sales')}
          className={`bg-white border rounded-2xl p-5 shadow-sm flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] ${
            activeSubTab === 'sales' ? 'border-emerald-500 ring-2 ring-emerald-500/10' : 'border-slate-200/80 hover:border-emerald-300'
          }`}
        >
          <div className="space-y-1">
            <div className="text-slate-500 text-xs font-semibold">ยอดขายรวมวันนี้</div>
            <div className="text-2xl font-black text-emerald-600">{totalRevenue.toLocaleString()} <span className="text-xs font-bold text-slate-400">฿</span></div>
          </div>
        ...
        <div
          onClick={() => setActiveSubTab('sales')}
          className={`bg-white border rounded-2xl p-5 shadow-sm flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] ${
            activeSubTab === 'sales' ? 'border-amber-500 ring-2 ring-amber-500/10' : 'border-slate-200/80 hover:border-amber-300'
          }`}
        >
          <div className="space-y-1">
            <div className="text-slate-500 text-xs font-semibold">ส่วนลดโปรโมชั่นรวม</div>
            <div className="text-2xl font-black text-amber-600">{totalDiscount.toLocaleString()} <span className="text-xs font-bold text-slate-400">฿</span></div>
          </div>
        ...
        <div
          ...
          className={`bg-white border rounded-2xl p-5 shadow-sm flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] ${
            activeSubTab === 'voids' ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200/80 hover:border-rose-300'
          }`}
        >
          <div className="space-y-1">
            <div className="text-slate-500 text-xs font-semibold">รายการยกเลิก (Void Logs)</div>
            <div className="text-2xl font-black text-rose-600">{totalVoidAmount.toLocaleString()} <span className="text-xs font-bold text-slate-400">฿ ({totalVoidCount})</span></div>
          </div>
                ...
                <div className="divide-y divide-slate-100">
                  {orders.map(order => (
                    <button
                      ...
                      className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50/80 transition-colors cursor-pointer group"
                    >
                      <div className="flex items-center gap-4">
                        {/* หมายเลขบิล */}
                        <div className="w-10 h-10 bg-slate-100 border border-slate-200 rounded-xl flex items-center justify-center">
                          <span className="text-xs font-extrabold text-slate-800">T{order.table_id}</span>
                        </div>

                        <div className="text-left">
                          <div className="text-xs font-extrabold text-slate-900">
                            ORD-{order.id}
                            <span className="text-slate-500 font-semibold ml-2">โต๊ะ {order.table_id}</span>
                          </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {order.promos.length > 0 && (
                          <div className="hidden sm:flex items-center gap-1.5">
                            {order.promos.slice(0, 2).map(promo => (
                              <span key={promo.id} className="text-[10px] font-bold text-red-700 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full truncate max-w-[120px]">
                                🏷️ {promo.promotion_name}
                              </span>
                            ))}
                            {order.promos.length > 2 && (
                              <span className="text-[10px] font-bold text-slate-400">+{order.promos.length - 2}</span>
                            )}
                          </div>
                        )}
                        ...
                        <div className="text-right">
                          ...
                          {order.payment && (
                            <div className="flex items-center justify-end gap-1 text-[10px] text-slate-500 font-semibold mt-0.5">
                  ...
                  {orders.length === 0 && (
                    <div className="py-16 text-center">
                      ...
                      <p className="text-slate-500 text-xs font-bold">ยังไม่มีบิลที่ปิดแล้วในวันนี้</p>
                      <p className="text-slate-400 text-[10px] mt-1 font-medium">บิลจะแสดงที่นี่หลังจากปิดการขายและชำระเงินเสร็จ</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4" onClick={() => setSelectedOrder(null)}>
          <div
            className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-xl"
          ...
          >
            ...
            <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
              <div>
                <h3 className="text-sm font-extrabold text-slate-900">ORD-{selectedOrder.id}</h3>
            ...
            {detailLoading ? (
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <div className="space-y-3">
                  <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">รายการอาหาร</h4>
                  <div className="space-y-2">
                    {orderItems.map(item => (
                      <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <span className="text-xs font-bold text-slate-900">{item.menu_items?.name || 'ไม่ทราบชื่อ'}</span>
                            <span className="text-[10px] text-slate-500 font-bold ml-2">x{item.quantity}</span>
                          </div>
                          <div className="text-right">
                ...
                {selectedOrder.promos.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">โปรโมชั่นที่ใช้</h4>
                    <div className="space-y-2">
                      {selectedOrder.promos.map(promo => (
                        <div key={promo.id} className="bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Tag className="w-3.5 h-3.5 text-red-600" />
                              <span className="text-xs font-bold text-red-800">{promo.promotion_name}</span>
                              <span className="text-[10px] font-bold text-red-600 bg-red-100 px-1.5 py-0.5 rounded">
                                {getPromoTypeLabel(promo.promotion_type)}
                              </span>
                            </div>
                            <span className="text-xs font-black text-rose-600">-{promo.discount_value.toLocaleString()} ฿</span>
                          </div>
                          {promo.free_items && promo.free_items.length > 0 && (
                            <div className="mt-2 space-y-1">
                              {promo.free_items.map((fi, idx) => (
                                <div key={idx} className="flex items-center gap-1.5 text-[10px] text-emerald-700 font-semibold">
                ...
                {selectedOrder.payment && (
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">สรุปยอดชำระ</h4>
                    <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 space-y-2.5">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-slate-800 font-bold">{selectedOrder.payment.subtotal.toLocaleString()} ฿</span>
                      </div>

                      {selectedOrder.promos.length > 0 && (
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">ส่วนลดโปรโมชั่น</span>
                      ...
                      {selectedOrder.payment.points_redeemed > 0 && (
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">ส่วนลดแต้มสะสม ({selectedOrder.payment.points_redeemed} แต้ม)</span>
                          <span className="text-rose-600 font-bold">-{selectedOrder.payment.points_redeemed.toLocaleString()} ฿</span>
                        </div>
                      )}
                      ...
                      <div className="border-t border-slate-200 pt-2.5 flex justify-between">
                        ...
                        <span className="text-sm font-black text-emerald-600">{selectedOrder.payment.net_amount.toLocaleString()} ฿</span>
                      </div>
                      ...
                      <div className="border-t border-slate-200 pt-2.5 flex items-center justify-between">
                        ...
                        {selectedOrder.payment.points_earned > 0 && (
                          <div className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">

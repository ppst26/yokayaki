...
import { Clock, CheckCircle, Check, Trash2, Loader2, Printer } from 'lucide-react';
import { VOID_REASONS, VOID_REASON_OTHER } from '@/lib/voidReasons';
import { Card } from '@/components/ui/card';
import type { KitchenOrderItem, KitchenTableGroup } from './types';
...
interface KitchenOrderCardProps {
  ...
  onPrintTicket: (group: KitchenTableGroup) => void;
}
...
export const KitchenOrderCard: React.FC<KitchenOrderCardProps> = ({
  ...
  const waitAnchor = pendingItems[0]?.created_at ?? group.oldest_created_at;
  const waitMinutes = hasPending ? getWaitTimeMinutes(waitAnchor) : 0;
  ...
  if (!hasPending) {
    barAccentStyle = 'bg-emerald-500 dark:bg-emerald-400';
    badgeStyle = 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-900/50';
  ...
  return (
    <Card className="overflow-hidden flex flex-col justify-between border border-slate-200 dark:border-neutral-800/80 bg-white dark:bg-neutral-900/90 shadow-sm rounded-2xl">
      <div>
        <div className="p-4 pb-3 flex items-center justify-between gap-2 border-b border-slate-100 dark:border-neutral-800/80">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className={`w-1 h-6 sm:h-7 rounded-full shrink-0 ${barAccentStyle}`} />
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-neutral-100 leading-tight truncate">
              ...
              <span className="text-xs text-slate-500 dark:text-neutral-400 font-medium block mt-0.5">
                ...
                {hasPending ? ` · ค้าง ${pendingItems.length}` : ' · เสิร์ฟครบ'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${badgeStyle}`}>
              ...
              <span>{hasPending ? `${waitMinutes} นาทีที่แล้ว` : 'เสิร์ฟครบ'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 divide-y divide-slate-100 dark:divide-neutral-800/70">
          {group.items.map(item => {
            ...
            return (
              <div key={item.id} className={`py-3 ${isServed ? 'opacity-80' : ''}`}>
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0 flex-1 pr-2">
                    <span
                      className={`font-bold text-sm sm:text-base block truncate ${
                        isServed
                  ...
                  <div className="flex items-center gap-2.5 shrink-0">
                    {isServed ? (
                      <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/50 px-2 py-1 text-[10px] sm:text-xs font-extrabold text-emerald-700 dark:text-emerald-300">
      ...
      <div className="p-4 pt-3">
        <button
          type="button"
          onClick={() => markAllTableItemsAsServed(pendingItems)}
          disabled={!hasPending}
          className="w-full py-3 bg-gradient-to-r from-red-600 via-red-700 to-red-800 hover:from-red-500 hover:to-red-700 disabled:from-slate-300 disabled:via-slate-300 disabled:to-slate-400 dark:disabled:from-neutral-700 dark:disabled:via-neutral-700 dark:disabled:to-neutral-700 disabled:cursor-not-allowed text-white font-bold text-sm sm:text-base rounded-xl transition-all duration-150 active:scale-98 flex items-center justify-center gap-2 cursor-pointer shadow-xs focus:outline-none focus-visible:outline-none select-none"
        >
          <CheckCircle className="w-4.5 h-4.5" />
          <span>
            {hasPending
              ? `เสิร์ฟทั้งหมดของโต๊ะ ${group.table_number || group.table_id}`
              : `โต๊ะ ${group.table_number || group.table_id} เสิร์ฟครบแล้ว`}
          </span>
        </button>
      </div>
    </Card>

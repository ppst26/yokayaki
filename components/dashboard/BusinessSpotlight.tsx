...
export const BusinessSpotlight: React.FC<BusinessSpotlightProps> = ({ bundle }) => {
  ...
  const paymentBreakdown = useMemo(() => {
    ...
    payments.forEach(p => {
      ...
      const cash = parseFloat(String(p.cash_amount)) || 0;
      const prompt = parseFloat(String(p.promptpay_amount)) || 0;
  ...
  const totalIngredientCost = useMemo(
    () => ingredients.reduce((s, i) => s + parseFloat(String(i.cost)), 0),
    [ingredients],
  );
    ...
    <div className="h-full flex flex-col justify-between gap-3 sm:gap-4">
      <Card className="p-5 space-y-3.5 flex-[1.4] flex flex-col justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-card-label">ยอดบัญชี</span>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-xs text-slate-400 dark:text-neutral-500 py-2 text-center">
      ...
      <Card className="p-4 sm:p-5 flex-[0.6] flex flex-col items-center justify-center text-center space-y-1">
        <div className="flex items-center gap-2 justify-center">
          ...
          <span className="text-card-label">ยอดต้นทุนวัตถุดิบ</span>
        </div>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-card-sublabel mt-0.5">ต้นทุนจัดซื้อในช่วงนี้ (ค่าใช้จ่าย)</p>
          </div>
        )}
      </Card>
    </div>
